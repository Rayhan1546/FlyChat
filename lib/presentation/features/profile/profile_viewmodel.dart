  ...
  Future<void> onClickSubmitButton(BuildContext context) async {
    ...
    if (name.isNotEmpty && image.value != null) {
      try {
        ...
        if (imageUrl != null) {
          bool response =
              await databaseRepository.insertDataIntoProfile(name, imageUrl, description);
          ...
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text('Failed to get image URL')));
        }
      } catch (e) {
        print('Error: $e');
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Name or image is empty')));
    }
  }
